...
def main(args):
    ...
    optimizer = optim.SGD(pg_model + pg_classifier, lr=args.lr, momentum=0.9, weight_decay=5e-5)  # 5e-5
    ...
    for epoch in range(args.epochs_begin+1, args.epochs):  # 训练
        ...
        for step, (inputs, labels, indexs) in enumerate(train_loader):
            model.train()
            classifier.train()
            optimizer.zero_grad()
            ...
            pbar.set_description(
                "[train epoch {}] loss: {:.3f}, loss_ce: {:.3f}, loss_ct: {:.3f}, acc: {:.3f}".format(epoch,
                                                                                                      train_loss,
                                                                                                      train_loss_ce,
                                                                                                      train_loss_ct,
                                                                                                      train_acc))
