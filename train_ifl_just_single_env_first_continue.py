...
from utils import evaluate, load_data_minerals, CenterLoss, try_gpu, compute_adjustment,\
    get_center_weight, update_env_by_score, ClassifierFC, CombinedModel, get_update_epoch
...
def main(args):
    ...
    scheduler = lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, epochs=args.epochs, steps_per_epoch=len(env1_loader),
    ...
    with warnings.catch_warnings():
        ...
        for epoch in range(args.epochs_begin + 1):  # 同步学习率
            for batch in range(len(env1_loader)):
                scheduler.step()
    ...
    weight_list1 = [(env1_loader.sampler.get_weight()).tolist()]  # 记录采样权重

    env_score_memo = {}

    update_epoch = get_update_epoch(args.epochs_begin, args.update_milestones)

    if update_epoch in args.update_milestones:
        print("更新环境 2")
        # 接续更新
        with open(f'all_ind_self_{update_epoch}.pkl', 'rb') as file:
            all_ind_self = pickle.load(file)
        with open(f'all_lab_self_{update_epoch}.pkl', 'rb') as file:
            all_lab_self = pickle.load(file)
        with open(f'all_prb_self_{update_epoch}.pkl', 'rb') as file:
            all_prb_self = pickle.load(file)
        with open(f'all_lgt_self_{update_epoch}.pkl', 'rb') as file:
            all_lgt_self = pickle.load(file)

        # save env_score
        env_score_memo['label_{}'.format(update_epoch)] = all_lab_self.tolist()
        env_score_memo['prob_{}'.format(update_epoch)] = all_prb_self.tolist()
        env_score_memo['idx_{}'.format(update_epoch)] = all_ind_self.tolist()
        ...
        weight_list1.append((env1_loader.sampler.get_weight()).tolist())
    ...
    for epoch in range(args.epochs_begin+1, args.epochs):  # 训练
        ...
        if args.lr_ct2 != 0.0 and epoch == args.center_milestones[-1]:  # 中心学习率跳变
            print(f"lr_ct: {center_optimizer.param_groups[0]['lr']}  ==>", end="")
            for param_group in center_optimizer.param_groups:
                param_group['lr'] = args.lr_ct2
            print(f"  {center_optimizer.param_groups[0]['lr']}")
        ...
        for step, ((inputs1, labels1, indexs1), (inputs2, labels2, indexs2)) in enumerate(zip(env1_loader, env2_loader)):
            ...
            pbar.set_description("[train epoch {}] loss: {:.3f}, loss_ce: {:.3f}, loss_ct: {:.3f}, acc: {:.3f}, "
                                 "acc_env1: {:.3f}, acc_env2: {:.3f}, lr: {}".format(epoch,
                                                                                     train_loss, train_loss_ce,
                                                                                     train_loss_ct,
                                                                                     train_acc, train_acc_env1,
                                                                                     train_acc_env2,
        ...
        if epoch in args.update_milestones:
            ...
            print(f'Finished Saving all_***_self_{epoch}')
    ...
    with open('weight_list1.pkl', 'wb') as file:
        pickle.dump(weight_list1, file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--center_weights', help='折衷参数 α', type=list, default=[0.0, 0.0003])  # [0.0, 0.001, 0.005]
    parser.add_argument('--center_milestones', help='折衷参数跳变点', type=list, default=[0, 60])  # [0, 60, 80]
    parser.add_argument('--update_milestones', help='环境更新点', type=list, default=[60, 65, 70, 75, 80, 85, 90, 95])  # [60, 80]
    ...
    parser.add_argument('--lr_ct2', type=float, default=0.0)  # 0.5
