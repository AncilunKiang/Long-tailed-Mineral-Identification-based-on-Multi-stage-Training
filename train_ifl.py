...
def main(args):
    ...
    classifier = ClassifierFC(feat_dim=feat_dim, num_classes=args.num_classes)  # 创建分类器
    ...
    optimizer = optim.SGD(pg_model + pg_classifier, lr=args.lr, momentum=0.9, weight_decay=5e-4)
    ...
    # 记录数据集编号
    # index_to_src = [env1_loader.dataset.imgs, env2_loader.dataset.imgs]
    # with open(f'index_to_src.pkl', 'wb') as file:
    #     pickle.dump(index_to_src, file)
    ...
    for epoch in range(args.epochs):  # 训练
        ...
        for step, ((inputs1, labels1, indexs1), (inputs2, labels2, indexs2)) in enumerate(zip(env1_loader, env2_loader)):
            ...
            # print("")  # 输出优化前梯度
            # loss_center_param_before = loss_center.centers.detach()
            # for name, param in loss_center.named_parameters():
            #     print('-->参数名:', name)
            #     print('-->参数值:', param)
            #     print('-->梯度计算:', param.requires_grad)
            ...
            # print("")  # 输出优化后梯度
            # print("更新后")
            # loss_center_param_after = loss_center.centers.detach()
            # for name, param in loss_center.named_parameters():
            #     print('-->参数名:', name)
            #     print('-->参数值:', param)
            #     print('-->梯度计算:', param.requires_grad)
            #     print('-->梯度值:', param.grad)
            # print(f"loss_center 参数是否未改变：{loss_center_param_before.equal(loss_center_param_after)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--center_weights', help='折衷参数 α', type=list, default=[0.0, 0.001, 0.005])
