...
class RandomCycleIter:
    """
    这个类用于生成一个可迭代的序列，每次迭代都会返回数据列表中的下一个元素。如果达到列表的末尾，它会重新开始，并在非测试模式下随机打乱数据列表。
    """
...
class PriorityTree(object):
    """
    这个类实现了一个二叉树结构，用于存储和更新权重。
    它支持两种权重：固定权重和自适应权重。固定权重在初始化时设置，不会随时间改变，而自适应权重可以根据需要进行更新。
    这个类还提供了更新权重、获取叶子节点和计算总和或最大值的方法。
    """
...
class ClassPrioritySampler(Sampler):
    """
    这个类是一个 PyTorch 的采样器，它结合了手动设计的采样策略和优先级采样策略。
    它支持多种采样策略，如平衡采样、均匀采样和基于优先级的采样。
    此外，它还支持在不同训练阶段改变采样策略，例如通过线性或周期性改变采样权重。
    """
    """
    ...
    def __init__(self, dataset, balance_scale=1.0, fixed_scale=1.0,
        ...
        self.balance_scale = balance_scale  # 用于平衡类别的权重比例
        self.fixed_scale = fixed_scale  # 固定权重的比例因子
        self.epochs = epochs
        self.lam = lam  # 控制平衡权重和均匀权重组合的参数
        self.cycle = cycle
        self.nroot = nroot  # 用于计算平衡权重的参数
        self.rescale = rescale  # 是否在每个周期重平衡手动权重和优先权重
        self.manual_only = manual_only  #
        self.root_decay = root_decay  # 控制根衰减的策略
        self.decay_gap = decay_gap  # 根衰减的间隔周期
        self.ptype = ptype  # 优先级采样的类型
        self.pri_mode = pri_mode  # 优先级采样的模式
        self.num_samples = len(dataset)  # 数据集中的样本数量
        self.manual_as_backend = False  # 是否将手动设计的权重作为优先级采样的后端分布
        self.momentum = momentum  # 用于更新权重的动量参数
        self.alpha = alpha  # 用于计算权重的指数
        ...
        self.cls_cnts = []
        self.labels = labels = np.array(self.dataset.targets)  # ImageFolder 通常没有直接的 labels,此处可使用 targets 替代
        ...
        for i, label in enumerate(self.dataset.targets):  # ImageFolder 通常没有直接的 labels,此处可使用 targets 替代
    ...
    def get_cls_ratios(self, tgt_weights):  # 计算每个类别的优先级比例。这通常基于手动设计的权重和后端分布（如果使用的话）
    ...
    def broadcast(self, per_cls_info):  # 将特定类别的信息广播到每个样本，以便在采样时考虑
    ...
    def debroadcast_sum(self, per_example_info):  # 计算特定类别的总和，这是从所有样本中广播特定类别的信息得到的
    ...
    def get_manual_weights(self, lam):  # 根据当前的 lambda 值，计算手动设计的权重。这结合了平衡权重和均匀权重
    ...
    def get_uniform_weights(self):  # 返回每个类别的均匀权重，这通常是每个类别的样本数量
        return self.cnts.copy()
    
    def get_balanced_weights(self, nroot):  # 计算每个类别的平衡权重。这通常基于每个类别的样本数量
    ...
    def reset_weights(self, epoch):  # 在训练的每个周期或根据根衰减策略重置采样权重
    ...
    def update_weights(self, inds, weights, labels):  # 根据每个批次的梯度更新采样权重。这有助于动态调整每个样本的采样概率
    ...
    def reset_priority(self, weights, labels):  # 根据每个批次的梯度更新采样权重。这有助于动态调整每个样本的采样概率
    ...
    def get_weights(self):  # 返回优先级树的权重
